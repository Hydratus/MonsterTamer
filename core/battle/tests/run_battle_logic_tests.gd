...
const LOGIC_TEST_LOGS_ENABLED := true

func _init() -> void:
	var results: Dictionary = BattleSmokeTestsClass.run_logic()
	var fails_only := _is_fails_only_enabled()
	var no_json := _is_no_json_enabled()
	_print_human_readable_results(results, fails_only)
	if not no_json:
		DEBUG_LOG.debug(LOGIC_TEST_LOGS_ENABLED, "LogicTestJSON", JSON.stringify(results))
...
func _print_human_readable_results(results: Dictionary, fails_only: bool) -> void:
	...
	if fails_only:
		DEBUG_LOG.debug(LOGIC_TEST_LOGS_ENABLED, "LogicTest", "Battle logic results (fails only):")
	else:
		DEBUG_LOG.debug(LOGIC_TEST_LOGS_ENABLED, "LogicTest", "Battle logic results:")
	for key in keys:
		var passed := bool(results.get(key, false))
		if passed:
			passed_count += 1
			if not fails_only:
				DEBUG_LOG.debug(LOGIC_TEST_LOGS_ENABLED, "LogicTest", "PASS - %s" % key)
		else:
			failed_count += 1
			DEBUG_LOG.debug(LOGIC_TEST_LOGS_ENABLED, "LogicTest", "FAIL - %s" % key)

	DEBUG_LOG.debug(LOGIC_TEST_LOGS_ENABLED, "LogicTest", "Summary: %d passed, %d failed, %d total" % [passed_count, failed_count, passed_count + failed_count])
